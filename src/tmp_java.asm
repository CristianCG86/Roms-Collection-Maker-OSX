...
LineData	equ	36					; 32 + 4 bytes de overhead
...
RomExec:
	...
	ld	de, RamPrgEnd - BankSel	; DE = offset hasta la variable
	add	hl, de				; HL = dirección de la variable

	ld	a,(SegMum)
	ld	(RamBottom+15),a			; Temporary SegMum

	ld	(hl), a				; Guardar A en esa dirección
...
PrintOK:
	...
	; *** NUEVO: Leer dirección RAM del juego ***
	; La dirección RAM está en hl+2 y hl+3 (después de segment y flags)
	push	hl
	inc	hl	; Saltar segment
	inc	hl	; Saltar flags
	ld	a, (hl)	; RAM_ADDR_LOW
	inc	hl
	ld	h, (hl)	; RAM_ADDR_HIGH
	ld	l, a	; HL = dirección RAM (low, high)
	ld	(RamLoc), hl
	pop	hl
